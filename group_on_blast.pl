...
push(@{$group{$blast[0]}},$blast[1],$blast[0]);
#$group{$blast[0]}{$blast[1]}=0;
...
foreach my$name(keys %group){
  #print "\nProcessing hits for $name";
      for (my$j=0;$j<=$i;$j++){
        $group = 'false';
        foreach my$subject(@{$group{$name}}){
          $group='true' if exists $final_group{$j}{$subject}

        }
         if ($group eq 'true'){$final_group{$j}{$_}=0 foreach @{$group{$name}};
                               #print "\n$name and hits belong to group $j";
                               }
          last if $group eq 'true';
      }
      if($group ne 'true'){$final_group{$i}{$_}=0 foreach @{$group{$name}};
                           #print "\n$name and hits belong to group $i";
                           $i++;
                           }
}
...
###### print all groups
my @group_names=keys %final_group;
my@colors=colourset(scalar keys %final_group);
my $k=0;
foreach my$fgroup(sort{$a<=>$b} keys %final_group){
  foreach my$fmemb(keys %{$final_group{$fgroup}}){
    print "\n$fmemb\t$colors[$k]";
    delete $all_seqs{$fmemb};
  }
  $k++;
}
#print "\nUngrouped Queries.\n";
foreach my$ungrouped(sort{$a cmp $b} keys %all_seqs){
  print "\n$ungrouped\t 255 255 255"
}
print "\n\n\n";
