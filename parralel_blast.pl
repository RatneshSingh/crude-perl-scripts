...
if($prg =~ /blastn|tblastx/i){
	
	if( ! -f "$database.nhr" && ! -f "$database.nal"){
		print "\nData base is not formatted. Formatting $database as Nucleotide database.";
		system("makeblastdb -in $database -dbtype nucl");
	} 
}else{
	if( ! -f "$database.phr" && ! -f "$database.pal"){
		print "\nData base is not formatted. Formatting $database as Protein database.\n";
		system("makeblastdb -in $database -dbtype prot"); 
	}
}
