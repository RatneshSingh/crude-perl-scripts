...
#################################################################################
';
our($opt_s,$opt_t,$opt_c,$opt_p,$opt_l);
$opt_c='yes';
$opt_t='arabidopsis';
$opt_p=1;
$opt_l=2000000;
...
while(<FASTA>){
	...
	if (length($sequence) > $split_len) {
        ...
        split_fasta(\$name,\$sequence,\$split_len,\%seq);
...
sub split_fasta{
	my($refname,$refsequence,$refsplit_len,$hRef_seq)=@_;
	...
	for(my$i=0;$i<=$seq_length;$i+=$$refsplit_len){
		my$start=$i;
