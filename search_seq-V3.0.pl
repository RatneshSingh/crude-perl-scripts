...
# for seperating multiple sequences in multiple files
	...
	while(<INFILE>){
			...
			if($opt_m =~/match/i){
				...
				if(scalar @{$ref_key_array} > 0){
					foreach my$header(@{$ref_key_array}){
						if($$seq_hash{$header}){print OUT2 ">$header\n$$seq_hash{$header}\n";
		        ...
		        if ($header){print OUT2">$header\n$sequence\n";
		            #~ print " Found Match for $ta\n";
		            $count_found++;
		        }
		        else{ $count_notfound++;$tcount_notfound++;push(@seqs_not_found,$name);
		        	print"Couldnot found Match for $name\n";
		        }
		...
		print "\nSaved $count_found sequences in $outputfile\n";
		print "Number of sequences not found: $count_notfound\n" if $count_notfound>0;


		close OUT2;
	}
		print "\nNumber of sequences found: $tcount_found\n";
		print "Number of sequences not found: $tcount_notfound\n" if $tcount_notfound>0;
		print join "\n",@seqs_not_found if $tcount_notfound>0;
...
sub searchPattern {
      ...
      foreach my $key (keys %{$seqhash}){
			...
			elsif (($mod_key =~ /^$mod_pattern[\D]+/ || $mod_key =~ /[\D]+$mod_pattern[\D]+/||$mod_key =~ /[\D]+$mod_pattern[\D]*$/) && $opt_m =~/match/i){ #use this line for matching pattern
             	print "For pattern:\t$pattern\t\tPicked seq-->\t$key\n" if $found==0;
				print "\t\t\t\t\t--->Mulitple match found\t-->\t$key\n" if $found==1;
				$found=1;
               	push(@seq_keys,$key);
