...
our($blastfile,$haplofile,$chrlen,$sample_order,$user_sample_only,$chr_order,$user_chr_only, $min_num_allele, $max_num_allele,,$chr_color, $help, $color_by_type_allele,$mar_list,%mar_list,$use_list_color,$invert_axis,$less_than_column_2,$more_than_column_2,$out_file,$print_allele_density,$marker_width,$alpha1,$print_consensus,$cons_color,$max_only,$add_line_start,$add_line_end,$add_line_chr,$add_line_samp,$merge_samples,$rgb_start,$rgb_end);
...
foreach my$sample(keys %haplos){
   foreach my$markers(keys %{$haplos{$sample}}){
      ...
      my$marker_color=defined $marker_color{$color_index}?$marker_color{$color_index}:$colors[$#colors];
        ...
        AddMarkersToChr(\$img,$m_x1,$m_y1,$marker_width,$marker_height,$marker_color,$marker_shape);
   }
}
