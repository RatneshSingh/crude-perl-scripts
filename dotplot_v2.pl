...
# Create and print the output.
    ...
    foreach(@contig_list[0..$cont_top-1]){
      ...
      $img->moveTo(($cumulative_contig{$_}{start} + (($contig_info{$_}{maxsize} - $zoom_mod)/2) + $font_size/2)/$nt_per_pix + $margin,
                   $margin-5);
    ...
    foreach(@lg_list[0..$lg_top-1]){
      ...
      $img->moveTo( int($margin/10),  ## x
                   ($cumulative_lg{$_}{start} + ($lg_info{$_}{maxsize}-$zoom_mod)/2 )/$cm_per_pix  + $margin + $font_size/2  ## y
      );
      $img->font('gdMediumBoldFont');
      $img->fontsize($font_size);
      $img->angle(0);
...
foreach my$marker(keys %all_info){
  print "\nSkipping Marker :$marker on contig:$all_info{$marker}{contig}" if  !$cumulative_contig{$all_info{$marker}{contig}}{start}  && $verbose;
    ...
    my$xvalue=($all_info{$marker}{loc}+ $cumulative_contig{$all_info{$marker}{contig}}{start} - $mod_zoom_contig)/$nt_per_pix  + $margin;
    my$yvalue=($all_info{$marker}{cm} + $cumulative_lg{$all_info{$marker}{lg}}{start} - $mod_zoom_lg)/$cm_per_pix  + $margin;
    my$color||='red';
