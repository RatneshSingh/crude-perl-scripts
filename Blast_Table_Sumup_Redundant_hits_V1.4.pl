...
our ( $opt_b, $opt_o, $opt_s, $help, $opt_pd, $opt_pc, $opt_v, $opt_and, $opt_or, $opt_e, $opt_l, $opt_p, $opt_qm, $opt_qx, $opt_sm, $opt_sx, $opt_final_qcoverage_max,
    $opt_final_qcoverage_min, $opt_final_length, $vectrim );
    ...
    if ( exists $blast{$Subject}{1}{'old_sstart'} ) {
        ...
        for ( my $hit = 1; $hit <= $uniq_hit{$Subject}{'hit'}; $hit++ ) {
            ...
            #2: check if end of new hit is in between the start and end of existing hit.
                ...
                #( ( $blast{$Subject}{$hit}{'old_sstart'} <= $current_end + $opt_a ) && ( $blast{$Subject}{$hit}{'old_sstart'} >= $current_start - $opt_a ) )
                #|| (   ( $blast{$Subject}{$hit}{'old_send'} <= $current_end + $opt_a )
                #    && ( $blast{$Subject}{$hit}{'old_send'} >= $current_start - $opt_a ) )
                #|| (   ( $current_start <= $blast{$Subject}{$hit}{'old_send'} + $opt_a )
                #    && ( $current_start >= $blast{$Subject}{$hit}{'old_sstart'} - $opt_a ) )
                #|| (   ( $current_end <= $blast{$Subject}{$hit}{'old_send'} + $opt_a )
                #    && ( $current_end >= $blast{$Subject}{$hit}{'old_sstart'} - $opt_a ) )
    ...
    for ( my $hit = 0; $hit < @hits; $hit++ ) {
        for ( my $nexthit = $hit + 1; $nexthit < @hits; $nexthit++ ) {
            ...
            if (
                ...
                #				# check if regions are within the distance of $opt_a and can be merged
                #                (
                #                    $blast{$subject}{ $hits[$hit] }{'old_send'} + $opt_a >= $blast{$subject}{ $hits[$nexthit] }{'old_sstart'}
                #                    &&    # if end of first element is less than the end of next element
                #                    $blast{$subject}{ $hits[$hit] }{'old_sstart'} <= $blast{$subject}{ $hits[$nexthit] }{'old_sstart'}
                #                )         # and larger than the start of next element
                #
                #                ||
                #
                #                # check if regions are within the distance of $opt_a and can be merged
                #                (
                #                    $blast{$subject}{ $hits[$hit] }{'old_send'} >= $blast{$subject}{ $hits[$nexthit] }{'old_sstart'}
                #                    &&    # if end of first element is less than the end of next element
                #                    $blast{$subject}{ $hits[$hit] }{'old_sstart'} - $opt_a <= $blast{$subject}{ $hits[$nexthit] }{'old_sstart'}
                #                )         # and larger than the start of next element
                #                ||
                #
                #                # sequences overlap
                #                # the first hit in comparison is before the next hit on the sequence
                #                (
                #                    $blast{$subject}{ $hits[$hit] }{'old_send'} <= $blast{$subject}{ $hits[$nexthit] }{'old_send'}
                #                    &&    # if end of first element is less than the end of next element
                #                    $blast{$subject}{ $hits[$hit] }{'old_send'} >= $blast{$subject}{ $hits[$nexthit] }{'old_sstart'}
                #                )         # and larger than the start of next element
                #
                #                ||
                #
                #                # OR if the first hit in comparison is after the next hit on the sequence.
                #                (
                #                    $blast{$subject}{ $hits[$hit] }{'old_sstart'} <= $blast{$subject}{ $hits[$nexthit] }{'old_send'}
                #                    &&    # if start of first element is less than the end of next element
                #                    $blast{$subject}{ $hits[$hit] }{'old_sstart'} >= $blast{$subject}{ $hits[$nexthit] }{'old_sstart'}
                #                )         # and start of first element is larger than the start of next element
                #                ||
                #
                #                # if sequence hit is inside next hit
                #                (
                #                    $blast{$subject}{ $hits[$hit] }{'old_sstart'} >= $blast{$subject}{ $hits[$nexthit] }{'old_sstart'}
                #                    &&    # if end of first element is less than the end of next element
                #                    $blast{$subject}{ $hits[$hit] }{'old_send'} <= $blast{$subject}{ $hits[$nexthit] }{'old_send'}
                ...
                #                ||
                #
                #                # if sequence next hit  is inside hit
                #                (
                #                    $blast{$subject}{ $hits[$hit] }{'old_sstart'} <= $blast{$subject}{ $hits[$nexthit] }{'old_sstart'}
                #                    &&    # if end of first element is less than the end of next element
                #                    $blast{$subject}{ $hits[$hit] }{'old_send'} >= $blast{$subject}{ $hits[$nexthit] }{'old_send'}
...
foreach my $subject ( keys %blast ) {
    foreach my $hit ( keys %{ $blast{$subject} } ) {
          ...
          )
        {
			...
			Skipping this hit as none of these are false
        ...
        if ($opt_s) {
            ...
            if ( $blast{$subject}{$hit}{'strand'} eq 'plus' ) {
                ...
                my $substr_start_site     = ( $blast{$subject}{$hit}{'old_sstart'} - 1 - $opt_h );
                my $substr_length_extract = ( $length + $opt_t + $opt_h );
                ...
                my $substr_start_site     = ( $blast{$subject}{$hit}{'old_sstart'} - 1 - $opt_t );
                my $substr_length_extract = ( $length + $opt_t + $opt_h );
            ...
            print {$fh_seqout}
              ">$subject Hit:$hit Region:$blast{$subject}{$hit}{'old_sstart'}..$blast{$subject}{$hit}{'old_send'} len:$length strand:$blast{$subject}{$hit}{'strand'} Total_length:$seq_length\n$seq\n"
              if $seq;
        }
...
print STDOUT "Finished printing uniq hits table in file $opt_b.$opt_k.summary.table\n"
  if ($opt_o);
print STDOUT "Saved sequences in file $opt_b.seq.fasta" if ( $opt_s && $opt_o );
