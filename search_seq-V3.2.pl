  ...
  chomp($ta);
	...
	if($opt_m =~/match/i){
	        ...
	        if ($header&&$sequence){print OUT">$header\n$sequence\n";
				...
				if(scalar @{$ref_key_array} > 0){
					foreach my$header(@{$ref_key_array}){
						if($$seq_fullhash{$header}){print OUT ">$header\n$$seq_fullhash{$header}\n";
...
sub searchPattern {
      ...
      foreach my $key (keys %{$seqhash}){
			...
			elsif (($mod_key =~ /^\s*$mod_pattern\s*$/ || $mod_key =~ /^$mod_pattern[\D]+/ || $mod_key =~ /[\D]+$mod_pattern[\D]+/||$mod_key =~ /[\D]+$mod_pattern[\D]*$/) && $matchtype =~/match/i){ #use this line for matching pattern
