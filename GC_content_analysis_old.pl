...
our (
    $opt_m,        $opt_n,        $opt_b, $opt_s,    $opt_o,      $opt_d,      $opt_f, $opt_r,       $opt_l,       $opt_w,      $opt_slide,
    $trim_at_stop, $start_at_atg, $frame, $gc_or_at, $which_base, $num_codons, $group, @group_lower, @group_upper, $min_length, ,$group_by, $help
);
...
-f	gcbin|randomgcbin|gcperfile|gcperseq|gc_sliding_window|GC3|GC3_window
...
-trim_at_stop|tas	yes|no. Trim ORF at stop codon if internal stop codons were found [no]
-frame				1|2|3. Which frame to look into for GC3 calculations[1]
-gc_or_at			GC|AT. count frequency for either GC or AT.
...
if ( !$opt_o ) {
    if ( scalar @files > 1 ) {
		...
		$opt_o = $opt_f . "_by_$group_by"."_" . $filename . "_and_" . $other_files_num . "_other_sequences.$opt_d.table" if uc $opt_f eq "GC3_POSITIONAL_GROUPED"
    }
    else {
        $opt_o = $opt_f . "_" . $filename . $opt_d . ".table";
		$opt_o = $opt_f . "_by_$group_by"."_" . $filename . $opt_d . ".table" if uc $opt_f eq "GC3_POSITIONAL_GROUPED";
    }
}
...
open OUT, ">$opt_o";
if ( uc $opt_f eq "GCBIN" || uc $opt_f eq "RANDOMGCBIN" || uc $opt_f eq "GC_SLIDING_WINDOW" || uc $opt_f eq "GC3" || uc $opt_f eq "GC3_POSITIONAL" ||uc $opt_f eq "GC3_POSITIONAL_GROUPED") {
...
if ( uc $opt_f eq "GC3_POSITIONAL_GROUPED" ) {
    ...
    if ( @group_lower && @group_upper ) {
        if ( scalar @group_lower ne scalar @group_upper ) {
            print "\n****Warning: Number of values in lower and upper are not equal. Number of groups created will be equal to the number of elements in smallest aray.\n";
        }
...
my %GC3_positional_grouped;
    ...
    for ( my $i = $opt_n; $i <= $opt_m; $i = $i + $opt_b ) {
        $GC{$file}{$i} = 0 if uc$opt_f ne "GC3_POSITIONAL_GROUPED";
    }
    my @GC_contents;
    my $print = 'no';
    ...
    # for calculating GC3 in coding sequences
        ...
        foreach ( keys %{$hSequence} ) {
            next if $_ =~ /^\s*$/;
            my $GC_content = count_GCn( $$hSequence{$_}, $frame, $start_at_atg, $trim_at_stop, $gc_or_at, $which_base );
            push( @GC_contents, $GC_content );
            for ( my $i = $opt_n; $i <= $opt_m; $i = $i + $opt_b ) {
                if ( $GC_content <= $i && $GC_content > $i - $opt_n ) { $GC{$file}{$i}++; }
            }
    ...
    # for calculating GC3 in coding sequences
        ...
        foreach ( keys %{$hSequence} ) {
            ...
            collect_positional_codons( $orf, $frame, \%GC3_positional );
    ...
    # for calculating GC3 in coding sequences
        ...
        foreach ( keys %{$hSequence} ) {
			...
			my$current_GCn=count_GCn( $orf, $frame, 'no', 'no', $gc_or_at, 3 );
			...
			for(my$i=0	;$i < $group_length;$i++){
				...
				if($current_GCn > $group_lower[$i] && $current_GCn <= $group_upper[$i]){
					collect_positional_codons_grouped( $orf, $frame, \%GC3_positional_grouped,$group_lower[$i],$group_upper[$i] );
    ...
    ###################################################################################################
    # calculate GC3 for each position from ATG start codon. codons are stored in array as hash value.
    # will print the result after this loop in next loop.
    if ( uc $opt_f eq "GC3_POSITIONAL" ) {
        ...
        foreach my $base_positions ( sort { $a <=> $b } keys %GC3_positional ) {
            ...
            my $GC_content = count_GCn( $virt_seq, $frame, 'no', 'no', $gc_or_at, $which_base );
...
sub count_GCn {
    ...
    if ( uc $GC_or_AT eq "AT" ) { $base1 = "A"; $base2 = "T" }
