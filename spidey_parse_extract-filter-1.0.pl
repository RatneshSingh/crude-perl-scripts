...
print "*****USAGE*****\n\nScript\tseqfile\tspidey_output_file\tgene_outputfile\tExon_number\n\n";

my (%seq_hash);

my$seqfile=$ARGV[0] or die "Provide Sequence file\n\n";;
my$spidey_output_file=$ARGV[1] or die "Provide Spidey output file\n\n";;
#my$gene_output="$seqfile.gene.out";
#my$summary_output="$seqfile.summary.out";
my$gene_output=$ARGV[2] or die "Provide gene outputfile\n\n";;
my$summary_output="$gene_output.summary";
my$exon_limit=$ARGV[3] or die "Provide exon number limit to filter\n\n";
$exon_limit=~s/\s//g;

open EXONGENE,">$gene_output.$exon_limit.exon.fasta";
...
#print "First line of the chunk is : $mRNA_info[0]\n";			
	...
	if($exon_N==$exon_limit){
		print EXONGENE">Gene_$mRNA\n$gene{$mRNA}\n" if defined $gene{$mRNA} ;
...
sub extract_gene{
	...
	print "\nGene:$exon_start,$exon_end,\tGenomic:$genomic,\tStrand:$strand\n\n\n";
