...
#  This script uses patmatch, download it first and save the same folder as this script
...
while(<PATTERN>){
	...
	foreach my $seqfile(@file_list){
		my @command=('perl scan_pipeline.pl',"-$opt_r",$pattern,$seqfile,$opt_M,$opt_t);
		#pop @command;
		my $command1=join(" ",@command,$process_output);
...
#	printf "Mean: %10.2f  Var: %10.2f  Std Dev: %10.2f  Trimmed mean:
#	%10.2f\n",
 #                               $mean,
  #                              $var ,
   #                             $std ,
