...
-feature|f		type of function to perform for the gene names in the list.[CDS]
		...
		'extract_repeats': Extract repeat regions from genomic sequences using .gff or .cat file from RepeatMasker.
...
-pattern|p		Name feature name to look for if feature names in gff3 file are not commonly used names.
...
# option processing
our (
    $opt_seqfile,     $opt_GFF_file,          $opt_gene_list_file, $opt_feature_type,          $opt_out_file,     $opt_delimiter,
    $opt_column,      $opt_upstream,          $opt_downstream,     $opt_run_modein_len_for_GC, $opt_feature_name, $opt_run_mode,
    $opt_min_rep_len, $opt_repeat_group_list, $help,               $print_raw
);
my ( @pattern, %Gene );
...
# Create list from the name of the sequence file
    ...
    foreach my $key ( keys %{$genome_seq} ) {
...
elsif ( lc $opt_feature_type eq 'extract_repeats'   && !$opt_feature_name ) { $opt_feature_name = 'similarity' }
...
foreach my $gene_name (@pattern) {
    my ( $gene_seq, $coords, $seqlen, $seqid, $gene_attributes );

    # to Mask the feature
    if ( lc $opt_feature_type eq 'mask' ) {
        ( $gene_seq, $coords, $seqlen, $seqid, $gene_attributes ) = get_seq( $gene_name, 'mask', $opt_feature_name );
        print {$fh} ">$gene_name Exon masked:$seqid$coords length:$seqlen \n$gene_seq\n" if $gene_seq ne "0";
    }

    # to extract the feature like: gene, cds, mRNA, exons, repeats
    elsif ( lc $opt_feature_type eq 'gene' || lc $opt_feature_type eq 'mrna' || lc $opt_feature_type eq 'cds' || lc $opt_feature_type eq 'exon' ) {
        ( $gene_seq, $coords, $seqlen, $seqid, $gene_attributes ) = get_seq( $gene_name, 'extract', $opt_feature_name );
    ...
    # to extract repeats from genomic DNA. will need .gff or .cat or .out file generated by repeatmasker.
    elsif ( lc $opt_feature_type eq 'extract_repeats' ) {
#        ( $gene_seq, $coords, $seqlen, $seqid, $gene_attributes ) = get_seq( $gene_name, 'extract', $opt_feature_name );
		my$gene_seq = get_repeat_seq( $gene_name, 'extract', $opt_feature_name );

        print {$fh} "$gene_seq" if (defined $gene_seq && $gene_seq ne "0");
    }
...
sub get_gene_info {
    ...
    my $filetype;              # vector to store file type(.gff or .cat) value
    open GFF, $gff_file or die "Cannot find gff file $opt_GFF_file";

    # if the GFF file is actually .cat or .out file (outputs from repeatmasker), then it should be processed differently

    my @tempFileName = split( /\./, $gff_file );
    if   ( $tempFileName[-1] =~ /cat/ ) { $filetype = 'cat' }
	elsif( $tempFileName[-1] =~/out/ ) { $filetype = 'out' }
    else                                { $filetype = 'gff' }

	#die "Filetype is detected as $filetype\n";

    #my$gene_name_new="\t$gene_name\t";
    # Pick all the lines in GFF file related to gene names.
    if ( $speed eq 'memory' ) {    # for memory eficient method. Is slow but will only store coordinates related to gene names.
        while (<GFF>) {
        ...
        foreach my $line (@selected_gff_lines) {
            ...
            if ( $filetype eq 'cat' ) {
				( $seqid, $source, $type, $start, $end, $score, $strand, $phase, $attributes ) = process_cat($line);

            }
			#process line from .out file from repeatmasker
            elsif ( $filetype eq 'out' ) {
				( $seqid, $source, $type, $start, $end, $score, $strand, $phase, $attributes ) = process_out($line);

            }
            # process line from gff file
            elsif ( $filetype eq 'gff' ) {
                ( $seqid, $source, $type, $start, $end, $score, $strand, $phase, $attributes ) = process_gff($line);
            }
            ...
            $Gene{$gene_name}{$type}{'attributes'} = $attributes;
    ...
    else {    # for speed efficient method. will read all the lines in gff file and store in memory. fast but very memory extensive..
        my $count;
        while (<GFF>) {
            ...
            $Gene{$seqid}{$type}{'attributes'} = $attributes;    # if ($type eq "mRNA");;
            $count++;
        }
...
sub get_seq {
    ...
    my $gene_attributes = $Gene{$gene_name}{$feature}{'attributes'};
    my $CDS_coord       = '(';

	print "GeneName:$gene_name, FeatureName:$feature, Genestart: $gene_start\n";
    ...
    return ( $gene_seq, $CDS_coord, $gene_length, $seq_id, $gene_attributes );

}


##########################################################################################################
# Get the repeat sequence of name gene_name
##########################################################################################################
sub get_repeat_seq {

    my $gene_name = shift;
    my $do_what   = shift;    # either 'mask' or 'extract'
    my $pattern   = shift;    # feature name
 my$fasta;

    chomp( $gene_name, $do_what);#, $feature );

	foreach my$feature(keys %{$Gene{$gene_name}}){

		next if $feature !~ m/$pattern/gi;
		...
		if ($gene_start <= 0 ) { return 0; }    # return zero if start position of the feature is not found.
		...
		if ( exists $$genome_seq{$seq_id} ) {
			...
			for ( my $i = 0; $i < scalar @{ $Gene{$gene_name}{$feature}{'startend'} } - 1; $i = $i + 2 ) {
				...
				$repeat_coord .= ${ $Gene{$gene_name}{$feature}{'startend'} }[$i] . '..' . ${ $Gene{$gene_name}{$feature}{'startend'} }[ $i + 1 ] . ',';
			}
		}
		else { print "Sequence with name $seq_id does not exists in the genomic file\n"; return 0; }

		$repeat_coord .= ')';
		$repeat_coord =~ s/,\)/\)/;    # remove last comma
		...
		$fasta.=">$gene_name $feature coord:$repeat_coord $gene_attributes\n$gene_seq\n";
	}
...
#print "\nExtracting Downstreamseq with following info\nStart:$start\nEnd:$end\nStrand:$strand\nDownstream:$downstream\nSequence:$$genome_seq{$seqid}\nSequence length:".length($$genome_seq{$seqid})."\nExtracted sequence;$downstream_seq\n";
        ...
        my (
            @seqid, $seqid,  @source, $source, @type,  $type,      @start,      $start,     @end,      $end, @score,
            $score, @strand, $strand, @phase,  $phase, @attribute, @attributes, $seqlength, @upstream, @downstream
        );
        for ( my $i = 0; $i < $number; $i++ ) {
            ( $seqid[$i], $source[$i], $type[$i], $start[$i], $end[$i], $score[$i], $strand[$i], $phase[$i], @attribute ) =
              split( /\t/, ${ $Gene{$sequence_name}{$feature}{'line'} }[$i] );
        ...
        for ( my $i = 0; $i < $number; $i++ ) {
            ...
            if ( $i == 0 ) {
                ...
                $downstream[$i] = ( min( $start[ $i + 1 ], $end[ $i + 1 ] ) - max( $start[$i], $end[$i] ) - 1 )
                ...
                $downstream[$i] = ( min( $start[$i], $end[$i] ) - 1 )
                  if ( ( $strand[$i] eq '-' || lc $strand[$i] eq 'minus' )
                    && ( min( $start[$i], $end[$i] - 1 ) < $downstream[$i] ) );

                $upstream[$i] = ( min( $start[ $i + 1 ], $end[ $i + 1 ] ) - max( $start[$i], $end[$i] ) - 1 )
                  if ( ( $strand[$i] eq '-' || lc $strand[$i] eq 'minus' )
...
#print "\n*****************End of the loop to process First element\nValues at the end of the loop are:\nstrand:$strand[$i]\nStart:$start[$i]\nEnd:$end[$i]\nUpstream:$upstream[$i]\nDownStream:$downstream[$i]\n";
            ...
            # for the last element
                ...
                $upstream[$i] = ( min( $start[$i], $end[$i] ) - max( $start[ $i - 1 ], $end[ $i - 1 ] ) - 1 )
                  if ( ( $strand[$i] eq '+' || lc $strand[$i] eq 'plus' )
                    && ( ( min( $start[$i], $end[$i] ) - max( $start[ $i - 1 ], $end[ $i - 1 ] ) - 1 ) < $upstream[$i] ) );

                $downstream[$i] = ( $seqlength - max( $start[$i], $end[$i] ) )
                  if ( ( $strand[$i] eq '+' || lc $strand[$i] eq 'plus' )
...
# for negative strand. requested downstream will be till the end of the previous element if distance between is smaller than reqested downstream.
                $downstream[$i] = ( min( $start[$i], $end[$i] ) - max( $start[ $i - 1 ], $end[ $i - 1 ] ) - 1 )
                  if ( ( $strand[$i] eq '-' || lc $strand[$i] eq 'minus' )
                    && ( ( min( $start[$i], $end[$i] ) - max( $start[ $i - 1 ], $end[ $i - 1 ] ) - 1 ) < $downstream[$i] ) );

                $upstream[$i] = $seqlength - max( $start[$i], $end[$i] )
                  if ( ( $strand[$i] eq '-' || lc $strand[$i] eq 'minus' )
...
sub summarize_gc_around_repeat {
    ...
    foreach ( keys %gc_array_summary ) {
        ...
        if ( $stat_element->count() != 0 ) { $gc_array_summary{$_}{'gcelement_mean'} = sprintf( "%0.2f", $stat_element->mean() ) }
        else                               { $gc_array_summary{$_}{'gcelement_mean'} = 'NoSeq' }
        if ( $stat_element->count() != 0 ) { $gc_array_summary{$_}{'gcelement_stdev'} = sprintf( "%0.2f", $stat_element->standard_deviation() ) }
        else                               { $gc_array_summary{$_}{'gcelement_stdev'} = 'NoSeq' }

        my $stat_up = Statistics::Descriptive::Full->new();
        $stat_up->add_data( @{ $gc_array_summary{$_}{'gcup'} } );
        if ( $stat_up->count() != 0 ) { $gc_array_summary{$_}{'gcup_mean'} = sprintf( "%0.2f", $stat_up->mean() ) }
        else                          { $gc_array_summary{$_}{'gcup_mean'} = 'NoSeq' }
        if ( $stat_up->count() != 0 ) { $gc_array_summary{$_}{'gcup_stdev'} = sprintf( "%0.2f", $stat_up->standard_deviation() ) }
        else                          { $gc_array_summary{$_}{'gcup_stdev'} = 'NoSeq' }

        my $stat_down = Statistics::Descriptive::Full->new();
        $stat_down->add_data( @{ $gc_array_summary{$_}{'gcdown'} } );
        if ( $stat_down->count() != 0 ) { $gc_array_summary{$_}{'gcdown_mean'} = sprintf( "%0.2f", $stat_down->mean() ) }
        else                            { $gc_array_summary{$_}{'gcdown_mean'} = 'NoSeq' }
        if ( $stat_down->count() != 0 ) { $gc_array_summary{$_}{'gcdown_stdev'} = sprintf( "%0.2f", $stat_down->standard_deviation() ) }
        else                            { $gc_array_summary{$_}{'gcdown_stdev'} = 'NoSeq' }

        my $stat_updown = Statistics::Descriptive::Full->new();
        $stat_updown->add_data( @{ $gc_array_summary{$_}{'gcupdown'} } );
        if ( $stat_updown->count() != 0 ) { $gc_array_summary{$_}{'gcupdown_mean'} = sprintf( "%0.2f", $stat_updown->mean() ) }
        else                              { $gc_array_summary{$_}{'gcupdown_mean'} = 'NoSeq' }
        if ( $stat_updown->count() != 0 ) { $gc_array_summary{$_}{'gcupdown_stdev'} = sprintf( "%0.2f", $stat_updown->standard_deviation() ) }
        else                              { $gc_array_summary{$_}{'gcupdown_stdev'} = 'NoSeq' }
        ...
        if   ( $stat_len_element->count() != 0 ) { $gc_array_summary{$_}{'len_element_sum'} = $stat_len_element->sum() }
        else                                     { $gc_array_summary{$_}{'len_element_sum'} = 'NoSeq' }
...
#$stat_len_updown->add_data(@{$gc_array_summary{$_}{'len_updown'}});
    ...
    foreach ( keys %gc_array_group_summary ) {
        ...
        if ( $stat_element->count() != 0 ) { $gc_array_group_summary{$_}{'gcelement_mean'} = sprintf( "%0.2f", $stat_element->mean() ) }
        else                               { $gc_array_group_summary{$_}{'gcelement_mean'} = 'NoSeq' }
        if ( $stat_element->count() != 0 ) { $gc_array_group_summary{$_}{'gcelement_stdev'} = sprintf( "%0.2f", $stat_element->standard_deviation() ) }
        else                               { $gc_array_group_summary{$_}{'gcelement_stdev'} = 'NoSeq' }

        my $stat_up = Statistics::Descriptive::Full->new();
        $stat_up->add_data( @{ $gc_array_group_summary{$_}{'gcup'} } );
        if ( $stat_up->count() != 0 ) { $gc_array_group_summary{$_}{'gcup_mean'} = sprintf( "%0.2f", $stat_up->mean() ) }
        else                          { $gc_array_group_summary{$_}{'gcup_mean'} = 'NoSeq' }
        if ( $stat_up->count() != 0 ) { $gc_array_group_summary{$_}{'gcup_stdev'} = sprintf( "%0.2f", $stat_up->standard_deviation() ) }
        else                          { $gc_array_group_summary{$_}{'gcup_stdev'} = 'NoSeq' }

        my $stat_down = Statistics::Descriptive::Full->new();
        $stat_down->add_data( @{ $gc_array_group_summary{$_}{'gcdown'} } );
        if ( $stat_down->count() != 0 ) { $gc_array_group_summary{$_}{'gcdown_mean'} = sprintf( "%0.2f", $stat_down->mean() ) }
        else                            { $gc_array_group_summary{$_}{'gcdown_mean'} = 'NoSeq' }
        if ( $stat_down->count() != 0 ) { $gc_array_group_summary{$_}{'gcdown_stdev'} = sprintf( "%0.2f", $stat_down->standard_deviation() ) }
        else                            { $gc_array_group_summary{$_}{'gcdown_stdev'} = 'NoSeq' }

        my $stat_updown = Statistics::Descriptive::Full->new();
        $stat_updown->add_data( @{ $gc_array_group_summary{$_}{'gcupdown'} } );
        if ( $stat_updown->count() != 0 ) { $gc_array_group_summary{$_}{'gcupdown_mean'} = sprintf( "%0.2f", $stat_updown->mean() ) }
        else                              { $gc_array_group_summary{$_}{'gcupdown_mean'} = 'NoSeq' }
        if ( $stat_updown->count() != 0 ) { $gc_array_group_summary{$_}{'gcupdown_stdev'} = sprintf( "%0.2f", $stat_updown->standard_deviation() ) }
        else                              { $gc_array_group_summary{$_}{'gcupdown_stdev'} = 'NoSeq' }

        # groutp stat for length
        my $stat_len_element = Statistics::Descriptive::Full->new();
        $stat_len_element->add_data( @{ $gc_array_group_summary{$_}{'len_element'} } );
        if   ( $stat_len_element->count() != 0 ) { $gc_array_group_summary{$_}{'len_element_sum'} = $stat_len_element->sum() }
        else                                     { $gc_array_group_summary{$_}{'len_element_sum'} = 'NoSeq' }
...
###################################################################################################
# process lines from gff and cat files
###################################################################################################
sub process_cat {

    my $line = shift;
	# Remove leading white spaces from line
	$line=~ s/^\s+//g;


    my ( $perc_div, $perc_del, $perc_ins, $query_remaining, $repeat_name, $repeat_name_class_family,$repeat_class_family, $rep_start, $rep_end, $rep_remaining, $insertion_ID, $seqid, $source, $type, $start,
        $end, $score, $strand, $phase, @attributes );

    # process line from .cat file
    # SW      perc   perc   perc  query      position in query     matching  repeat         position in  repeat
    # score   div.   del.   ins.  sequence   begin  end  (left)    repeat    class/family    begin end (left)       uniq_insertion_ID
    if ( $line =~ m/\sC\s/ ) {
        (
            $score, $perc_div, $perc_del, $perc_ins, $seqid, $start, $end, $query_remaining, $strand, $repeat_name_class_family, $rep_start, $rep_end, $rep_remaining,
            $insertion_ID
        ) = split( /\s+/, $line );
        $strand = '-';
    }
    else {
        ( $score, $perc_div, $perc_del, $perc_ins, $seqid, $start, $end, $query_remaining, $repeat_name_class_family, $rep_start, $rep_end, $rep_remaining, $insertion_ID )
          = split( /\s+/, $line );
        $strand = '+';
    }

    @attributes = (
        'Repeat Name:',               $repeat_name, ', Repeat Class/Family:',     $repeat_class_family, ', Repeat insertion ID:',       $insertion_ID,
        ', Repeat_percent_divergence:', $perc_div,    ', Repeat_percent_deletion:', $perc_del,            ', Repeat_percent_insertions:', $perc_ins
    );
    my $attributes = join( '', @attributes );
	($repeat_name,$repeat_class_family)=split(/\#/,$repeat_name_class_family);
    $source = 'RepeatMasker';
    $type   = $insertion_ID.'/'.$repeat_name.'/'.$repeat_class_family;
#	$type   = $insertion_ID;
	$phase='.';
	$strand='-' if $strand eq 'C';

	#print "SeqID:$seqid, Source:$source, Type:$type, Start:$start, End:$end, Score:$score, Strand:$strand, Phase:$phase, Attributes:$attributes\n";
...
sub process_out {
    ...
    my ( $perc_div, $perc_del, $perc_ins, $query_remaining, $repeat_name, $repeat_class_family, $rep_start, $rep_end, $rep_remaining, $insertion_ID, $seqid, $source, $type, $start,
        $end, $score, $strand, $phase, @attributes );
    ...
    # score   div.   del.   ins.  sequence   begin  end  (left)    repeat    class/family    begin end (left)       uniq_insertion_ID
    (
        $score, $perc_div, $perc_del, $perc_ins, $seqid, $start, $end, $query_remaining, $strand, $repeat_name, $repeat_class_family, $rep_start, $rep_end, $rep_remaining,
        $insertion_ID
    ) = split( /\s+/, $line );
