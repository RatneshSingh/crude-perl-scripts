...
=> to calculate the OE ratio for random selected fragments. \nMultiple sequences in sequence file will be joined into \none large molecule an will be used for selecting random fragments

	perl script [options...........]
	
-f	file containing multiple sequences
-r	Only test the OE ratio in randomaly selected sequences [false]
	(Exclusion of this option will calculate in all the sequence in file)
...
if(defined $opt_f){
	my$ouputfile="OEratio_".$opt_f.".txt";
	open OUT,">$ouputfile";}

elsif(!defined $opt_f){
	if($ARGV[0]){my$outputfile="OEratio_".$ARGV[0].".txt"; open OUT,">$outputfile";}
	else{print "Please provide sequence file to calculate OE ratio\n\n\n"; exit;}
}
...
my$count;
if(!defined $opt_r || lc($opt_r) eq lc('false')){
	# for individuals files with one sequence in each file
	if(!defined $opt_f){
		
		# determineif to remove ambigous bases from sequence
		my $ambigous;
		if(!defined $opt_a||lc($opt_a) eq lc('false')){$ambigous=0}
		elsif(lc($opt_a) eq lc('true')){$ambigous=1}
		else{print "Please provide 'True' or 'False' as option for flag '-a'. \n Assuming 'False' for option '-a'\n\n"; $ambigous=0;}
	...
	# When one file with multiple sequence is provided
		...
		my%sequences= ReadFastaTohash($opt_f,$ambigous);
	...
	# determineif to remove ambigous bases from sequence
	my $ambigous;
	if(lc($opt_a) eq lc('false')|| !defined $opt_a){$ambigous=0}
	elsif(lc($opt_a) eq lc('true')){$ambigous=1}
	else{print "Please provide 'True' or 'False' as option for flag '-a'. \n Assuming 'False' for option '-a'\n\n"; $ambigous=0;}
		...
		my$sequence1= ReadFasta($opt_f,$ambigous); # join multiple sequence to one
...
#	print "reading Sequences from input file.....Plz wait...\n";
	...
	while(<FASTA>){
    	...
    	$sequence=~ s/\s//g;
    	$sequence=~s/\n//g;
    	if($amb==1){$sequence=~s/N//gi;}	# remove ambigous bases if asked for
    	if($header=~/^\s*$/){next;}
 		$sequence2.=$sequence;
	}
